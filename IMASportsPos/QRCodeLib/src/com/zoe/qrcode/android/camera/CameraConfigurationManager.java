...
package com.zoe.qrcode.android.camera;
...
import com.zoe.qrcode.custom.ConfigHandler;
...
final class CameraConfigurationManager {
	 ...
	 */
	void initFromCameraParameters(Camera camera, Rect reck) {
		...
		screenResolution = new Point(reck.right - reck.left, reck.bottom
				- reck.top);
	...
	void setDesiredCameraParameters(Camera camera, boolean safeMode) {
		...
		parameters.setPictureFormat(ImageFormat.JPEG); // Õº∆¨–Œ Ω
		parameters.set("jpen-quality", 80);
		CameraConfigurationUtils.setFocus(parameters,
				ConfigHandler.CameraAutoFocus(),
				ConfigHandler.CameraContinuousFocus(), safeMode);

		if (!safeMode) {
			if (ConfigHandler.CameraInvertScan()) {
				CameraConfigurationUtils.setInvertColor(parameters);
			}

			if (!ConfigHandler.CameraDisableBarcodeSceneMode()) {
				CameraConfigurationUtils.setBarcodeSceneMode(parameters);
			}

			if (!ConfigHandler.CameraDisableMetering()) {
	...
	private void initializeTorch(Camera.Parameters parameters, boolean safeMode) {
		boolean currentSetting = ConfigHandler.CameraFrontLightMode() == FrontLightMode.ON;
	...
	private void doSetTorch(Camera.Parameters parameters, boolean newSetting,
		...
		if (!safeMode && !ConfigHandler.CameraDisableExposure()) {
